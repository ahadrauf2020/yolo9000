...
parser.add_argument('--imagenetTrain', type=str, default='tiny-imagenet-200/train', help='ImageNet train root')
parser.add_argument('--imagenetVal', type=str, default='tiny-imagenet-200/val', help='ImageNet val root')
...
# traindir = os.path.join(args.imagenetTrain)
# valdir = os.path.join(args.imagenetVal)

print('===> Loading datasets')

if opt.mode == 'train':
    train_set = ImageFolderWithPaths(root = opt.imagenetTrain, transform = data_transform)
...
def train(epoch):
    ...
    # for itr, (image, label),(paths,_) in enumerate(training_data_loader, 1):
    for itr, ((image, label), (paths, _)) in enumerate(training_data_loader, 1):
        image_class_dict = {}
        for i in range(len(image)):
            # image_class_dict[image[i]] = paths[i].split('/')[2:3]
            image_class_dict[paths[i]] = image[i]
            # print(image_class_dict[image])
        if itr > MaxIter:
            break

        if opt.target == -1:
            # least likely class in nontargeted case
            pretrained_label_float = pretrained_clf(image)
        ...
        recons = torch.add(image, delta_im)
        ...
        #     k = 0
        #     for path, im in image_class_dict.items():
        #         print(im)
        #         print(image[i])
        #         if torch.abs(im - image[i]):
        #     # print(image[i])
        #             torchvision.utils.save_image(recons[i], opt.expname+'/train/reconstructed_train_{}_{}_{}.png'.format(path, itr,k))
        #             k += 1
        for i in recons:
            torchvision.utils.save_image(recons, opt.expname+'/train/reconstructed_train_{}_{}.png'.format(itr,k))
        output_pretrained = pretrained_clf(recons)
...
def test():
    ...
    for itr, (image, class_label) in enumerate(testing_data_loader):
        ...
        outputs_recon = pretrained_clf(recons)
        outputs_orig = pretrained_clf(image)
        _, predicted_recon = torch.max(outputs_recon, 1)
        _, predicted_orig = torch.max(outputs_orig, 1)
        total += image.size(0)
        correct_recon += (predicted_recon == class_label).sum()
        correct_orig += (predicted_orig == class_label).sum()
        ...
        if itr % 1 == 0:
            ...
            for i in recons:
                k = 0
                torchvision.utils.save_image(i, opt.expname+'/val/reconstructed_val_{}_{}_{}.png'.format(class_label, itr, k))
                k+=1
            # torchvision.utils.save_image(recons, opt.expname+'/reconstructed_val_{}.png'.format(itr))
