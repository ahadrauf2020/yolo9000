...
model = resnet_modified.resnet152(pretrained=True, decay_factor=0.0)
...
#                 with torch.set_grad_enabled(phase == 'train'):
        ...
        if save:
            torch.save(model.state_dict(), './models/' + model_name + '_best_model_epoch_' + str(epoch + start_count) + '.pth')
            np.save('training_in_progress_' + model_name + '.npy', np.array([tr_acc, val_acc, tr_loss, val_loss]))
...
#                                                        num_epochs=10, verbose=True, model_name="resnet152_demo")
    
#torch.save(model.state_dict(), './models/resnet152_demo_best_model_state_dict_10.pth')
#np.save('resnet152_demo_training_10.npy', np.array([tr_acc, val_acc, tr_loss, val_loss]))



print("Starting blurred")

# Load Data from folders
image_datasets = {
    'train': datasets.ImageFolder(os.path.join(data_dir, 'train_adversarial'), transform=data_transforms),
    'val': datasets.ImageFolder(os.path.join(data_dir, 'val'), transform=data_transforms),
    'test': datasets.ImageFolder(os.path.join(data_dir, 'test'), transform=data_transforms)
}
...
model, tr_acc, val_acc, tr_loss, val_loss = train_model(model, criterion, optimizer_ft, scheduler, dataloaders, 
                                                        num_epochs=10, verbose=True, model_name="resnet152_demo_blurred")
    
torch.save(model.state_dict(), './models/resnet152_demo_blurred_best_model_state_dict_10.pth')
np.save('resnet152_demo_blurred_training_10.npy', np.array([tr_acc, val_acc, tr_loss, val_loss]))
...
# np.save("train_data_resnet152_demo_v1.npy", np.array(train_data))
# np.save("val_data_resnet152_demo_v1.npy", np.array(val_data))
